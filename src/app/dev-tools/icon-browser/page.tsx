...
const allLucideIcons: { name: string; component: LucideIcon }[] = Object.entries(lucideIconCollection)
...
export default function IconBrowserPage() {
  ...
  const IconDisplay = ({ icon, onClick, isSelected }: { icon: { name: string; component: LucideIcon }, onClick: () => void, isSelected: boolean }) => {
    ...
    return (
      ...
      >
        ...
        <span className="text-xs text-center truncate w-full block">{icon.name}</span>
      </button>
    );
  };
  ...
  return (
    <div className="flex flex-col h-full">
      <PageHeader
        ...
        description={isClient ? `Browse and search through ${allLucideIcons.length} Lucide icons.` : "Browse and search Lucide icons."}
        icon={Sparkles}
      />
        ...
        <Card className="md:col-span-2 flex flex-col min-h-0">
          ...
          <CardContent className="flex-grow p-4 pt-0 overflow-hidden">
            {isClient && filteredIcons.length === 0 && searchTerm.trim() && (
             ...
             {isClient && filteredIcons.length === 0 && !searchTerm.trim() && allLucideIcons.length > 0 && (
              <div className="text-center text-muted-foreground py-10">
        ...
        <Card className="md:col-span-1 flex flex-col min-h-0">
          ...
          <CardContent className="flex-grow flex flex-col items-center justify-center p-4 text-center space-y-3">
            {isClient && selectedIcon ? (
              <>
                <selectedIcon.component size={56} className="text-primary mb-2" />
                <p className="text-base font-semibold">{selectedIcon.name}</p>
                <div className="space-y-2 w-full max-w-xs">
                    <Button 
                        onClick={() => handleCopyToClipboard(selectedIcon.name, 'Icon Name')}
                        variant="outline"
                    ...
                    <Button 
                        onClick={() => handleCopyToClipboard(`import { ${selectedIcon.name} } from 'lucide-react';`, 'Import Statement')}
                        variant="outline"
                        className="w-full text-sm"
                        size="sm"
                    >
                ...
                <CardDescription className="text-xs pt-2">
                    Pass props like <code className="bg-muted/70 text-accent-foreground px-1 py-0.5 rounded font-code">size</code>, <code className="bg-muted/70 text-accent-foreground px-1 py-0.5 rounded font-code">color</code>, <code className="bg-muted/70 text-accent-foreground px-1 py-0.5 rounded font-code">strokeWidth</code>.
                </CardDescription>
              </>
            ) : (
