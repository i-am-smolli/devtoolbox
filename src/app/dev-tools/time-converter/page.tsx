...
import {
  format, parse, formatISO, parseISO, getUnixTime, fromUnixTime,
  addYears, subYears, addMonths, subMonths, addDays, subDays,
...
const initialFieldsDefinition: Record<TimeFormat, Omit<InputField, 'value' | 'error'>> = {
  ...
  dateOnly: { id: 'dateOnly', label: 'Date (YYYY-MM-DD)', placeholder: 'e.g., 2023-03-15' },
  timeOnly: { id: 'timeOnly', label: 'Time (HH:mm:ss)', placeholder: 'e.g., 10:30:00' },
  isoWeek: { id: 'isoWeek', label: 'ISO Week Date', placeholder: 'e.g., 2023-W11-3' },
  http: { id: 'http', label: 'HTTP Date (RFC1123)', placeholder: 'Wed, 15 Mar 2023 10:30:00 GMT' },
...
export default function TimeConverterPage() {
  ...
  const formatFieldValue = useCallback((date: Date | null, fieldId: TimeFormat): string => {
    ...
    try {
      switch (fieldId) {
        ...
        case 'unixMilliseconds': return date.getTime().toString();
        case 'iso8601': return formatISO(date);
        case 'dateOnly': return format(date, 'yyyy-MM-dd');
        case 'timeOnly': return format(date, 'HH:mm:ss');
        case 'isoWeek': return format(date, "yyyy-'W'II-i");
  ...
  useEffect(() => {
    ...
    setFieldStates(prevStates => {
      ...
      for (const key in newStates) {
        ...
        if (newStates[fieldId].value !== formattedValue || newStates[fieldId].error) {
          newStates[fieldId] = { ...newStates[fieldId], value: formattedValue, error: null };
          changed = true;
        }
      }
  ...
  if (!isClient) {
    return (
      <div>
        <PageHeader title="Time Converter" description="Convert between various date/time formats." icon={Clock} />
        <Card><CardContent className="p-6 text-center">Loading converter...</CardContent></Card>
      </div>
    );
  }
      ...
      <Card>
        <CardHeader>
          ...
          <div className="mt-4 p-3 bg-muted rounded-md">
            ...
            <p className="font-code text-lg">{localTimeDisplay || 'Calculating...'}</p>
          </div>
        </CardHeader>
              ...
              {tinkerButtonsConfig.map(tb => (
                <div key={tb.label} className="space-y-1">
                  ...
                  <div className="flex gap-1">
                    <Button onClick={() => adjustTime(tb.addFn, 1)} variant="outline" size="icon" aria-label={`Add 1 ${tb.label}`} className="h-8 w-8"> <Plus className="h-4 w-4" /> </Button>
                    <Button onClick={() => adjustTime(tb.subFn, 1)} variant="outline" size="icon" aria-label={`Subtract 1 ${tb.label}`} className="h-8 w-8"> <Minus className="h-4 w-4" /> </Button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
            {Object.values(fieldStates).map(field => (
              <div key={field.id} className="space-y-1">
                <Label htmlFor={field.id} className={field.error ? 'text-destructive' : ''}>{field.label}</Label>
                <Input
                  id={field.id}
                  type="text"
                  value={field.value}
