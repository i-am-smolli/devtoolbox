...
export default function UuidGeneratorPage() {
  ...
  const handleGenerateUuids = () => {
    ...
    if (isNaN(count) || count < MIN_COUNT || count > MAX_COUNT) {
      setError(`Number of UUIDs must be between ${MIN_COUNT} and ${MAX_COUNT}.`);
      return;
    }
  ...
  const handleCopyToClipboard = async () => {
    ...
    try {
      ...
      toast({
        title: "UUIDs Copied!",
        description: "The generated UUID(s) have been copied to your clipboard.",
      });
    } catch (err) {
      toast({
        title: "Copy Failed",
    ...
    <div className="flex flex-col space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="uuidCount">Number of UUIDs to Generate (Max: {MAX_COUNT})</Label>
            <Input
              id="uuidCount"
              type="number"
              value={count}
          ...
          <Button onClick={handleGenerateUuids} disabled={isLoading || !isClient}>
      ...
      {isClient && generatedUuids && !error && (
        <Card>
          ...
          <CardFooter>
            <Button onClick={handleCopyToClipboard} variant="outline" disabled={!generatedUuids}>
