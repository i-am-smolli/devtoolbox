...
interface CidrCalculationResults {
  ...
  wildcardMask: string;
  cidrNotation: string;
  ipType: 'Private' | 'Public' | 'Loopback' | 'Link-Local' | 'Reserved' | 'Multicast' | 'Invalid';
  originalIp: string;
  prefix: number;
}
...
function getIpType(ipLong: number): CidrCalculationResults['ipType'] {
  ...
  if (firstOctet === 172 && ((ipLong >>> 16) & 0xFF) >= 16 && ((ipLong >>> 16) & 0xFF) <= 31) return 'Private'; // 172.16.0.0/12
  if (firstOctet === 192 && ((ipLong >>> 16) & 0xFF) === 168) return 'Private'; // 192.168.0.0/16
  if (firstOctet === 169 && ((ipLong >>> 16) & 0xFF) === 254) return 'Link-Local'; // 169.254.0.0/16
...
export default function CidrCalculatorPage() {
  ...
  const [calculationResults, setCalculationResults] = useState<CidrCalculationResults | null>(null);
  ...
  const parseCidr = useCallback((cidrStr: string): { ipLong: number; prefix: number; originalIp: string } | null => {
  ...
  const calculateCidrDetails = useCallback((ipLong: number, prefix: number, originalIp: string): CidrCalculationResults => {
    const maskLong = prefix === 0 ? 0 : (-1 << (32 - prefix)) >>> 0;
    const networkLong = (ipLong & maskLong) >>> 0;
    const broadcastLong = (networkLong | (~maskLong >>> 0)) >>> 0;
    
    const numUsableHosts = prefix <= 30 ? Math.pow(2, 32 - prefix) - 2 : (prefix === 31 ? 0 : (prefix === 32 ? 0 : 0));
    const firstUsableHostLong = prefix <= 30 ? networkLong + 1 : networkLong;
    const lastUsableHostLong = prefix <= 30 ? broadcastLong - 1 : broadcastLong;
  ...
  const handleCalculateCidr = useCallback(() => {
    ...
    const results = calculateCidrDetails(parsed.ipLong, parsed.prefix, parsed.originalIp);
    setCalculationResults(results);
    setSubnettingBaseCidr(results.cidrNotation); // Pre-fill subnetting base
    setNewPrefixLengthInput( (parsed.prefix < 30 ? parsed.prefix + 1 : parsed.prefix).toString() ); // Suggest next prefix
  }, [cidrInput, parseCidr, calculateCidrDetails]);
  
  useEffect(() => {
    if (isClient && cidrInput) { // Auto-calculate on initial load or if cidrInput changes programmatically
        handleCalculateCidr();
    }
  ...
  const handleGenerateSubnets = useCallback(() => {
    ...
    if (isNaN(newPrefix) || newPrefix <= baseParsed.prefix || newPrefix > 32) {
      setSubnetError(`New prefix length must be a number greater than ${baseParsed.prefix} and less than or equal to 32.`);
      return;
    }
    ...
    let currentNetworkLong = baseParsed.ipLong & ((-1 << (32 - baseParsed.prefix)) >>> 0); // Ensure base network address

    for (let i = 0; i < numberOfSubnets; i++) {
      const subnetDetails = calculateCidrDetails(currentNetworkLong, newPrefix, longToIp(currentNetworkLong));
      subnets.push({
  ...
  }, [subnettingBaseCidr, newPrefixLengthInput, parseCidr, calculateCidrDetails]);
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-end gap-2">
            ...
            <Button onClick={handleCalculateCidr} className="w-full sm:w-auto">Calculate</Button>
          </div>
          ...
          {isClient && calculationResults && !error && (
            <div className="space-y-3 pt-4">
              <h3 className="text-lg font-semibold">Calculation Results for {calculationResults.originalIp}/{calculationResults.prefix}</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                <div><strong>Network Address:</strong> <span className="font-code">{calculationResults.networkAddress}</span></div>
                <div><strong>Broadcast Address:</strong> <span className="font-code">{calculationResults.broadcastAddress}</span></div>
                <div><strong>First Usable Host:</strong> <span className="font-code">{calculationResults.firstUsableHost}</span></div>
                <div><strong>Last Usable Host:</strong> <span className="font-code">{calculationResults.lastUsableHost}</span></div>
                <div><strong>Number of Usable Hosts:</strong> <span className="font-code">{calculationResults.numUsableHosts.toLocaleString()}</span></div>
                <div><strong>Subnet Mask:</strong> <span className="font-code">{calculationResults.subnetMask}</span></div>
                <div><strong>Wildcard Mask:</strong> <span className="font-code">{calculationResults.wildcardMask}</span></div>
                <div><strong>CIDR Notation:</strong> <span className="font-code">{calculationResults.cidrNotation}</span></div>
                <div><strong>IP Type:</strong> <span className="font-code">{calculationResults.ipType}</span></div>
              </div>
            </div>
          )}
        </CardContent>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-end gap-2">
            ...
            <Button onClick={handleGenerateSubnets} className="w-full sm:w-auto">Generate Subnets</Button>
          </div>
              ...
              <Table>
                ...
                <TableBody>
                  {generatedSubnets.map((subnet, index) => (
                    <TableRow key={index}>
                      <TableCell className="font-code">{subnet.networkAddress}</TableCell>
                      <TableCell className="font-code">{subnet.cidrNotation}</TableCell>
                      <TableCell className="font-code">{subnet.usableHostRange}</TableCell>
                      <TableCell className="font-code">{subnet.broadcastAddress}</TableCell>
                      <TableCell className="text-right font-code">{subnet.numUsableHosts.toLocaleString()}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
