...
function parseCronPart(partValue: string, partIndex: number): string {
  ...
  if (partValue === "?") {
    if (partIndex === 2 || partIndex === 4) { // Day of Month or Day of Week
      return "Any value (no specific value)";
    } else {
      throw new Error(`'?' can only be used for Day of Month or Day of Week in ${partName} part.`);
    }
  }
  ...
  if (partValue.includes("*/")) {
    ...
    if (/^\d+$/.test(step) && parseInt(step) >= 1) {
      ...
      // Basic validation for step against range (e.g. */70 for minutes is not ideal)
       if ( (partIndex === 0 && stepNum > 59) || (partIndex === 1 && stepNum > 23)) {
      ...
      throw new Error(`Invalid step value in ${partName} part: "${partValue}". Must be a positive number.`);
    }
  }

  if (partValue.includes(",")) {
    const values = partValue.split(",");
    if (values.every(v => /^\d+$/.test(v) && parseInt(v) >= range.min && parseInt(v) <= range.max)) {
      return `At ${partName}(s) ${values.join(", ")}`;
    } else {
      throw new Error(`Invalid values in ${partName} part: "${partValue}". Ensure all are numbers within range [${range.min}-${range.max}].`);
    }
  }
  ...
  if (partValue.includes("-")) {
    ...
    if (/^\d+$/.test(start) && /^\d+$/.test(end) &&
        parseInt(start) >= range.min && parseInt(end) <= range.max && parseInt(start) <= parseInt(end)) {
      return `From ${partName} ${start} through ${end}`;
    } else {
      throw new Error(`Invalid range in ${partName} part: "${partValue}". Ensure start <= end and within range [${range.min}-${range.max}].`);
    }
  }
  ...
  if (/^\d+$/.test(partValue)) {
    ...
    if (numVal >= range.min && numVal <= range.max) {
      ...
      throw new Error(`Invalid value in ${partName} part: "${partValue}". Must be a number within range [${range.min}-${range.max}].`);
    }
  }
  ...
  if (partIndex === 3 && /^[a-zA-Z]{3}$/.test(partValue)) { // Month abbreviations like JAN, FEB
    return `In month ${partValue.toUpperCase()}`;
  }
  if (partIndex === 4 && /^[a-zA-Z]{3}$/.test(partValue)) { // Day abbreviations like SUN, MON
...
export default function CronParserPage() {
  ...
  const handleParseCron = () => {
    ...
    if (parts.length < 5 || parts.length > 6) { // Allow 6 parts for optional year, though we only parse 5
      setError("Invalid cron expression: Must have 5 (or 6, for year) parts separated by spaces.");
      return;
    }
    ...
    try {
      const newParsedParts: ParsedCronPart[] = standardParts.map((partValue, index) => ({
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          <div className="space-y-2">
            ...
            <div className="flex items-center space-x-2">
             ...
             <p className="text-xs text-muted-foreground">
              ...
              Year field (6th part) is ignored if present. Some advanced symbols (L, W, #) are not fully interpreted.
            </p>
          </div>
          ...
          {parsedParts && !error && (
            <Card>
              <CardHeader>
                <CardTitle className="font-headline text-lg">Parsed Cron Details</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                {parsedParts.map((part) => (
                  <div key={part.label} className="text-sm">
                    <span className="font-semibold">{part.label}</span>
                    <span className="text-muted-foreground font-code"> ({part.originalValue}): </span>
