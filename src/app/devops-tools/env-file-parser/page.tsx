            ...
            <ScrollArea className="h-full w-full p-4 min-h-[200px]">
              ...
              {!error && parsedVariables.length === 0 && !envContent.trim() && (
                <p className="text-muted-foreground">
                  Paste your .env content in the input area to see parsed variables.
                </p>
              )}
              {!error && parsedVariables.length === 0 && envContent.trim() && (
                <p className="text-muted-foreground">
                  No variables found or the input is empty after parsing comments.
                </p>
              )}
                  ...
                  <TableBody>
                    {parsedVariables.map((variable) => (
                      <TableRow key={variable.key}>
                        <TableCell className="font-medium font-code break-all">{variable.key}</TableCell>
                        <TableCell className="font-code break-all">{variable.value}</TableCell>
                        <TableCell className="text-right">
                          <Button
                            variant="outline"
                            size="icon"
                            onClick={() => handleCopyToClipboard(variable.value, variable.key)}
