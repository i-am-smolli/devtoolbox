...
export default function CronExpressionBuilderPage() {
  ...
  useEffect(() => {
    const expressionParts = CRON_PARTS.map((part) =>
      buildPartExpression(configs[part.id]),
    );
              ...
              >
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      handleModeChange(part.id, value as CronPartMode)
                    }
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="every">
                        Every {part.name.toLowerCase()} (*)
                      </SelectItem>
                      <SelectItem value="specific">
                        Specific {part.name.toLowerCase()}(s)
                      </SelectItem>
                      <SelectItem value="range">{part.name} Range</SelectItem>
                      <SelectItem value="step">
                        Every X {part.name.toLowerCase()}(s) (Step)
                      </SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                ...
                {configs[part.id].mode === "specific" && (
                  <div className="space-y-2">
                    <Label htmlFor={`${part.id}-specific`}>
                      Specific Value(s) (comma-separated)
                    </Label>
                    <Input
                      id={`${part.id}-specific`}
                      placeholder={
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        id={`${part.id}-rangeEnd`}
                        placeholder={
                          part.id === "minute" || part.id === "dayOfWeek"
                            ? parseInt(part.range.split("-")[1], 10).toString()
                            : part.range.split("-")[1].match(/\d+/)?.[0] || ""
                        }
                ...
                {configs[part.id].mode === "step" && (
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        handleConfigChange(part.id, "stepValue", e.target.value)
                      }
                      className="font-code"
                    />
                  </div>
