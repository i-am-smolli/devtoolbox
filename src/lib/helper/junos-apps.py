...
def parse_junos_app_config(config_text):
    """
    Parses Junos SRX application configuration and extracts name, protocol, and port.
    Example input:
    ...
    Returns:
        List of dicts: [{'name': ..., 'protocol': ..., 'port': ...}, ...]
    """
    apps = []
    app_blocks = re.findall(r'application\s+(\S+)\s*\{([^}]*)\}', config_text, re.MULTILINE | re.DOTALL)
    for name, block in app_blocks:
        protocol_match = re.search(r'^\s*protocol\s+(\S+);', block, re.MULTILINE)
        port_match = re.search(r'destination-port\s+(\S+);', block)
        protocol = protocol_match.group(1) if protocol_match else None
        port = port_match.group(1) if port_match else None
        apps.append({'name': name, 'protocol': protocol, 'port': port})
    return apps
...
application junos-pop3 {
    application-protocol pop3;
    protocol tcp;
    destination-port 110;
}
application junos-pop3s {
    application-protocol pop3s;
    protocol tcp;
...
application junos-vnc {
    ...
    term t2 protocol tcp destination-port 5900;
}
#
...
#
# secure smtp application
#
application junos-smtps {
    term t1 alg smtps protocol tcp destination-port 587;
    term t2 alg smtps protocol tcp destination-port 465;
}
#
...
application-set junos-sun-rpc-any {
    ...
    """
    result = parse_junos_app_config(config)
    for app in result:
